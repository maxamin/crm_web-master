...
<div class="row">
    <div class="col-md-4">
        ...
        <?php endif; ?>
        <?php if ($model->contactsLinks0): ?>
            <div class="card card-collapsed">
                <div class="card-head">
                    <header>Связанные юр. лица</header>
                    <div class="tools">
                        <div class="btn-group">
                            <a class="btn btn-icon-toggle btn-collapse"><i class="fa fa-angle-up"></i></a>
                        </div>
                ...
                <div class="card-body no-padding" style="display: none;">
                    <ul class="list">
                        <?php foreach ($model->contactsLinks0 as $key => $value): ?>
                            <li id="link-<?= $key ?>" class="tile">
                                <a class="tile-content ink-reaction"
                                   href="<?= Url::to(['@' . $value->link->getTypeLabel() . '-view', 'id' => $value->link->id]); ?>">
                                    <div class="tile-text">
                                        <?= $value->link->name ?>
                                        <small>ОКПО: <?= $value->link->okpo ?></small>
                                    </div>
                                </a>
                                <a class="btn btn-flat ink-reaction btn-unlink" href="javascript:void(0);"
                                   data-href="<?= Url::to(['unlink', 'id' => $value->id]) ?>"
                                   data-name="<?= Html::encode($value->link->name) ?>"
                                   data-container-selector="#link-<?= $key ?>">

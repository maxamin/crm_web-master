...
class LegalSearch extends Model
{
    ...
    public function search($params)
    {
        ...
        $query->andFilterWhere([
            'and',
            ['=', 'legal.active', $this->active ?? $this->getActiveValueByStatus($this->status)],
            ['=', 'legal.c_user_id', $this->cid ?? $this->getCidValueByOwn($this->own)],
            ['=', 'legal.r_user_id', $this->rid ?? $this->getRidValueByOwn($this->own)],
            ['=', 'rBranch.code', $this->branch],
            ['>=', 'legal.created_at', $this->from],
            ['<', 'legal.created_at', $this->to],
        ]);

        $query->andFilterWhere([
            (isset($this->q) && $this->q !== '') ? 'or' : 'and',
            ['like', 'legal.name', $this->name ?? $this->q],
            ['like', 'legal.description', $this->description ?? $this->q],
            ['=', 'legal.okpo', $this->okpo ?? $this->q],
    ...
    private function getActiveValueByStatus($status = null)
    {
        ...
        switch ($status) {
            case self::STATUS_ACTIVE:
                $active = Legal::STATUS_ACTIVE;
                break;
            case self::STATUS_INACTIVE:
                $active = Legal::STATUS_INACTIVE;
                break;
        }
